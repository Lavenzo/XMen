...
class Player extends Actor {
  ...
  attack(type) {
    ...
    if (id === "storm") {
      ...
      range =
        this.def.range + (heavy ? 55 : 10) + (this.specialTime > 0 ? 50 : 0);
    } else {
...
class Scenery {
  ...
  draw(c) {
    ...
    if (g.stage.index === 0 && stage1Background.naturalWidth > 0) {
    ...
    } else if (g.stage.index === 3 && stage4Background.naturalWidth > 0) {
      const scale = H / 724;
      ...
      c.drawImage(stage4Background, offset, 0, scaledW, H);
      c.drawImage(stage4Background, offset + scaledW, 0, scaledW, H);
    } else if (g.stage.index === 4 && stage5Background.naturalWidth > 0) {
      const scale = H / 724;
      const scaledW = 2172 * scale;
